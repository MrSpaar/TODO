...
if [ -x "$(command -v apt)" ]; then
  ...
  sudo apt install -y cmake make g++ libgtk-3-dev libsqlite3-dev libnotify || printf "${RED}Could not install dependencies${NC}\n" || exit 1
  ...
  sudo dnf install cmake make gcc-g++ gtkmm3.0-devel sqlite-devel libnotify-devel -y || printf "${RED}Could not install dependencies${NC}\n" || exit 1
  ...
  sudo pacman -S cmake make gcc gtkmm3 sqlite libnotify --noconfirm || printf "${RED}Could not install dependencies${NC}\n" || exit 1
elif [ -x "$(command -v yum)" ]; then
  printf "${GREEN}Installing dependencies with yum${NC}\n"
  sudo yum update || printf "${RED}Could not update package list${NC}\n" || exit 1
  sudo yum install cmake make gcc-c++ gtkmm3.0-devel sqlite-devel libnotify-devel -y || printf "${RED}Could not install dependencies${NC}\n" || exit 1
else
